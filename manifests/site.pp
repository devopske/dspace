...
exec { "Create Database for : ${title}": 
   ...
   unless  => "test \$(psql --host=${db_endpoint} --port=5432  --username=${db_user} DspaceDb -tA -c \"SELECT count(*)=1 FROM pg_catalog.pg_database where datname='${db_name}';\") = t",
#onlyif  => "test \$(psql --host=${db_endpoint} --port=5432  --username=${db_user} DspaceDb -tA -c \"SELECT count(*)=1 FROM pg_catalog.pg_database where datname='${db_name}';\") = t",
